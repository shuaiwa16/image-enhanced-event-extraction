...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    with tf.variable_scope("loss"):
        ...
        # mask = tf.tile(tf.expand_dims(mask,-1),multiples=[1,1,38])
...
def main(_):
    ...
    # early_stopping = tf.contrib.estimator.stop_if_no_decrese_hook(
    #     estimator,
    #     metric_name='loss',
    #     max_steps_without_decrease=2,
    #     min_steps=0)
    ...
    if FLAGS.do_train:
        ...
        result=estimator.train_and_evaluate(
            train_spec=estimator.train(input_fn=train_input_fn, max_steps=num_train_steps),
            eval_spec=estimator.evaluate(input_fn=eval_input_fn, steps=eval_steps))
