...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    with tf.variable_scope("loss"):
        ...
        # output_layer=tf.concat([tf.zeros_like(output_layer[:,:FLAGS.WINDOW,:]),output_layer],1)
        # output_layer=tf.concat([output_layer,tf.zeros_like(output_layer[:,:FLAGS.WINDOW,:])],1)
        # for index in range(FLAGS.WINDOW,FLAGS.WINDOW+max_seq_length):#right-left padding, split stack randomly
        #     current=output_layer[:,index-FLAGS.WINDOW:index+FLAGS.WINDOW,:]
        #     current=
        ########dense_layer
        ...
        # log_likelihood, transition_params = tf.contrib.crf.crf_log_likelihood(logits, labels,tf.reduce_sum(input_mask,axis=-1))
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            logging_hook = tf.train.LoggingTensorHook({"loss per epoch" : total_loss}, every_n_iter=int(num_train_steps/int(FLAGS.num_train_epochs)))
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        output_eval_file = os.path.join(FLAGS.input_dir, "eval_results.txt")
