...
flags.DEFINE_integer('pic_num_classes', 10,"class number of picture")
...
def create_tf_example(image, label, resize_size=None):
    image=image.reshape(3,32,32)
    image=image.reshape(32,32,3)
    # tf.logging.info("##########uuuuuuu:",image.dtype)
    image=PIL.Image.fromarray(image)
    width, height = image.size
    bytes_io = io.BytesIO()
    image.save(bytes_io, format='JPEG')
    encoded_jpg = bytes_io.getvalue()
    return encoded_jpg,label,height,width
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer,annotation_dict,mode):
    ...
    #random pick picture
    index=np.random.randint(0,len(annotation_dict),size=1)
    encoded_jpg,label,height,width = create_tf_example(annotation_dict['data'][index[0]],annotation_dict['label'][index[0]])
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    cls_model = modeling_picture.Model(is_training=True,num_classes=FLAGS.pic_num_classes)
